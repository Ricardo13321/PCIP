...
//Iniciando algumas variáveis da sessão
$_SESSION['id'] = [];
...
if (isset($_GET['data_inicial']) && isset($_GET['data_final'])) {
    ...
    $_SESSION['data-final'] = $_GET['data_final'];
} else {
    $_SESSION['data_inicial'] = date('Y/m/d');
    $_SESSION['data_final'] = date('Y/m/d');
}

$_SESSION['data_inicial_split'] = explode('/', $_SESSION['data_inicial']);
$_SESSION['data_final_split'] = explode('/', $_SESSION['data_final']);

foreach ($_SESSION["date"] as $key => $value) {
    $datesplit = explode("/", $value);
    
    if($datesplit[2] >= $_SESSION['data_inicial_split'][0] && $datesplit[2] <= $_SESSION['data_final_split'][0]) {
    }
}
